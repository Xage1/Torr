...
async function autoScroll(page) {
    ...
    while (sameCountRounds < 10) {
        ...
        await delay(2500);
        // @ts-ignore - browser-side code
        const adCount = await page.evaluate(() => document.querySelectorAll(".b-list-advert-base.b-list-advert-base--list.qa-advert-list-item").length);
...
async function scrapeListing(page) {
    ...
    return page.evaluate(() => {
        const ads = [];
        // @ts-ignore
        const adEls = document.querySelectorAll(".b-list-advert-base.b-list-advert-base--list.qa-advert-list-item");
        // @ts-ignore
        adEls.forEach((el) => {
            ...
            const description = el.querySelector(".b-list-advert-base__description-text")
                ?.textContent?.trim() || "";
...
async function scrapeAdImages(page, url) {
    try {
...
async function enhanceImage(url, savePath) {
    try {
...
(async () => {
    ...
    const existingFolders = new Set(fs
    ...
    const toScrape = ads.filter((ad) => {
        ...
        return (!scrapedTitles.has(cleanTitle) &&
